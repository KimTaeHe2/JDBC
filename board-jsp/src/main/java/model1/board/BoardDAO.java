...
public class BoardDAO extends JDBConnect{
	...
	public List<BoardDTO> selectList(Map<String,Object> map){
		...
		if (map.get("searchWord") != null) {
			// 검색어가 있으면
			query += "where " + map.get("searchField") + " like '%" + map.get("searchWord") + "%'"; // 정렬 기준 추가
			// searchField : 제목,내용,작성자 
			// searchWord : input text로 넘어온 글자
			// select count(*) from board where 제목 like '%딸기%' ;
		} // 검색어가 있으면 조건이 추가 된다.
	...
	// 메서드 호출시 입력은 num(board pk) 받고 가져온 데이터를 BoardDTO객체에 넣어 리턴
		...
		// member에 있는 작성자를 가져올수 있도록 join 처리용
		String query = "select B.*, M.name from member M inner join board B on M.id = B.id where num=? ";
		// member 테이블의 별칭은 M 으로 board 테이블의 별칭은 B로 선언
		// 부모테이블은 M에 이너 조인 B를 이용하고 id가 같은 자료를 찾음 M.id = B.id
		// 조건은 파라미터로 받은 bno를 이용
		// 찾아온 값은 board에 모든 것 과 member의 name을 가져옴. -> dto에 name 필드를 추가
