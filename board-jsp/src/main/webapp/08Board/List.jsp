...
<%
	...
	int totalCount = boardDAO.selectCount(param); //검색조건을 파리미터로 dao로 넘어가고 게시물 수를 int로 받음 1단계
	// 전체 페이지 수 계산
	int pageSize = Integer.parseInt(application.getInitParameter("POSTS_PER_PAGE")); // 현재 페이지에 보여줄 리스트 개수(10)
	int blockPage = Integer.parseInt(application.getInitParameter("PAGES_PER_BLOCK")); // 한 화면에 보여줄 블럭 수(5)
	...
	<table border="1" width="90%">
		<tr align="center">
			<td>
				...
				<!--                        105        10         5          1         List.jsp?pageNum=1  -->
				<!--   1  2  3  4  5 [다음 블록] [마지막 페이지]       -->
			</td>
			<td>
